...
class DWT_3D(Module):
    ...
    def get_matrix(self):
        ...
        # Match the actual input, not global CUDA availability/default GPU.
        options = dict(device=self._device, dtype=self._dtype)
        self.matrix_low_0 = torch.tensor(matrix_h_0, **options)
        self.matrix_low_1 = torch.tensor(matrix_h_1, **options)
        self.matrix_low_2 = torch.tensor(matrix_h_2, **options)
        self.matrix_high_0 = torch.tensor(matrix_g_0, **options)
        self.matrix_high_1 = torch.tensor(matrix_g_1, **options)
        self.matrix_high_2 = torch.tensor(matrix_g_2, **options)
    ...
    def forward(self, input):
        ...
        self.input_height = input.size()[-2]
        self.input_width = input.size()[-1]
        self._device, self._dtype = input.device, input.dtype
        self.get_matrix()
...
class IDWT_3D(Module):
    ...
    def forward(self, LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH):
        ...
        self._device, self._dtype = LLL.device, LLL.dtype
        self.get_matrix()
