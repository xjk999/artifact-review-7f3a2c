...
class WavUNetModel(nn.Module):
    """
    重构后的WavUNet模型，使用清晰的模块化结构
    channel_mult=(1, 2, 2, 4, 4, 4) 表示6个level
    每个encoder level: 2个ResBlock + 1个down + 1个wavdown
    每个decoder level: 2个ResBlock + 1个up
    """

    def __init__(self, image_size=256, in_channels=16, model_channels=64, out_channels=8,
                 num_res_blocks=2, attention_resolutions=(), dropout=0.0,
                 channel_mult=(1, 2, 2, 4, 4, 4), conv_resample=True, dims=3,
                 ...
                 bottleneck_attention=False, resample_2d=False,
                 additive_skips=False, decoder_device_thresh=0,
                 use_freq=True, progressive_input='residual'):
        super().__init__()
        ...
        # Level 0: ch = 64 (1 * 64)
        self.enc_block1_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels, time_embed_dim, dropout, model_channels * channel_mult[0],
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block1_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[0], time_embed_dim, dropout, model_channels * channel_mult[0],
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[0], time_embed_dim, dropout,
                model_channels * channel_mult[0],  # 64→64，通道数不变
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                down=True, num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_wav_down1 = TimestepEmbedSequential(
            WaveletDownsample(in_ch=in_channels, out_ch=model_channels * channel_mult[0])
        )

        # Level 1: ch = 128 (2 * 64)
        self.enc_block2_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[0], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 64→128，通道数改变
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block2_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.enc_wav_down2 = TimestepEmbedSequential(
            WaveletDownsample(
                in_ch=model_channels * channel_mult[0],
                out_ch=model_channels * channel_mult[1]
            )
        )

        # Level 2: ch = 128 (2 * 64)
        self.enc_block3_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,
                model_channels * channel_mult[2],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block3_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[2],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down3 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[2],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.enc_wav_down3 = TimestepEmbedSequential(
            WaveletDownsample(
                in_ch=model_channels * channel_mult[1],
                out_ch=model_channels * channel_mult[2]
            )
        )

        # Level 3: ch = 256 (4 * 64)
        self.enc_block4_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 128→256，通道数改变
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block4_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down4 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.enc_wav_down4 = TimestepEmbedSequential(
            WaveletDownsample(
                in_ch=model_channels * channel_mult[2],
                out_ch=model_channels * channel_mult[3]
            )
        )

        # Level 4: ch = 256 (4 * 64)
        self.enc_block5_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block5_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down5 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.enc_wav_down5 = TimestepEmbedSequential(
            WaveletDownsample(
                in_ch=model_channels * channel_mult[3],
                out_ch=model_channels * channel_mult[4]
            )
        )

        # Level 5: ch = 256 (4 * 64) - 最后一层也有下采样！
        self.enc_block6_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_block6_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.enc_down6 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.enc_wav_down6 = TimestepEmbedSequential(
            WaveletDownsample(
                in_ch=model_channels * channel_mult[4],
                out_ch=model_channels * channel_mult[5]
            )
        )

        # ==================== BOTTLENECK (MIDDLE) BLOCKS ====================
        self.middle_block1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.middle_block2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        # ==================== DECODER BLOCKS ====================
        # Decoder结构：每个level有3个操作：ResBlock×2 + up
        # 通道数变化在每个level的第一个ResBlock，up操作通道数不变

        # Level 5 Decoder: ch = 256
        self.dec_block6_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_block6_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_up6 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[5],  # 256→256，通道数不变！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                up=True, num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )

        # Level 4 Decoder: ch = 256
        self.dec_block5_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[5], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_block5_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_up5 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[4],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        # Level 3 Decoder: ch = 256
        self.dec_block4_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[4], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_block4_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 256→256
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_up4 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,
                model_channels * channel_mult[3],  # 256→256，通道数不变！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                up=True, num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )

        # Level 2 Decoder: ch从256→128
        self.dec_block3_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[3], time_embed_dim, dropout,  # 输入256
                model_channels * channel_mult[2],  # 256→128，通道数变化！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.dec_block3_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[2],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_up3 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[2],  # 128→128，通道数不变！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                up=True, num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )

        # Level 1 Decoder: ch = 128
        self.dec_block2_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[2], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_block2_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 128→128
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        self.dec_up2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,
                model_channels * channel_mult[1],  # 128→128，通道数不变！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                up=True, num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )

        # Level 0 Decoder: ch从128→64
        self.dec_block1_1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[1], time_embed_dim, dropout,  # 输入128
                model_channels * channel_mult[0],  # 128→64，通道数变化！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.dec_block1_2 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[0], time_embed_dim, dropout,
                model_channels * channel_mult[0],  # 64→64
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.dec_up1 = TimestepEmbedSequential(
            ResBlock(
                model_channels * channel_mult[0], time_embed_dim, dropout,
                model_channels * channel_mult[0],  # 64→64，通道数不变！
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
        ...
        # ==================== OUTPUT BLOCKS ====================
        self.out_res1 = TimestepEmbedSequential(
            ResBlock(
                model_channels, time_embed_dim, dropout, model_channels,  # 64→64
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
            )
        )
        self.out_res2 = TimestepEmbedSequential(
            ResBlock(
                model_channels, time_embed_dim, dropout, model_channels,  # 64→64
                dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm,
                num_groups=num_groups, resample_2d=resample_2d, use_freq=use_freq
        ...
        self.out = nn.Sequential(
            normalization(model_channels, num_groups),
            nn.SiLU(),
            conv_nd(dims, model_channels, out_channels, 3, padding=1),  # 64→8
        )


        bottleneck_channels = model_channels * channel_mult[-1]
        self.dissection_head = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),  # 全局平均池化 -> [B, C, 1, 1, 1]
            nn.Flatten(),  # -> [B, C]
            nn.Linear(bottleneck_channels, 128),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(128, 1),  # 二分类输出 (使用BCEWithLogitsLoss)
        )
    def forward(self, x, timesteps):
        """
        Forward pass
        ...
        Args:
            x: [N x 16 x D x H x W] input tensor
            timesteps: [N] timestep tensor

        Returns:
            output: [N x 8 x D x H x W] output tensor
        """
        # ct_condition = x[:, 8:, ...]
        # Generate timestep embedding
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        # Store skip connections (high-frequency components from wavelet transform)
        skips = []

        # ==================== ENCODER ====================
        # Initial convolution: 16→64
        h = self.init_conv(x, emb)  # [N, 64, D, H, W]
        input_pyramid = x
        skip = None
        skips.append(skip)

        # Level 0: 64→64
        h = self.enc_block1_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block1_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down1(input_pyramid, emb)
        h = input_pyramid + h
        input_pyramid = h

        # Level 1: 64→128
        h = self.enc_block2_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block2_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down2(input_pyramid, emb)
        h = input_pyramid + h
        input_pyramid = h

        # Level 2: 128→128
        h = self.enc_block3_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block3_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down3(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down3(input_pyramid, emb)
        h = input_pyramid + h
        input_pyramid = h

        # Level 3: 128→256
        h = self.enc_block4_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block4_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down4(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down4(input_pyramid, emb)
        h = input_pyramid + h
        input_pyramid = h

        # Level 4: 256→256
        h = self.enc_block5_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block5_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down5(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down5(input_pyramid, emb)
        h = input_pyramid + h
        input_pyramid = h

        # Level 5: 256→256
        h = self.enc_block6_1(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_block6_2(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        h = self.enc_down6(h, emb)
        if isinstance(h, tuple):
            h, skip = h
        skips.append(skip)

        input_pyramid = self.enc_wav_down6(input_pyramid, emb)
        h = input_pyramid + h
        ...
        bottleneck_feat = h
        # ==================== DECODER ====================
        # 维护一个持久的skip变量，只有当pop出非None值时才更新
        current_skip = None
        ...
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block6_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block6_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up6(h, emb)
        ...
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block5_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block5_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up5(h, emb)
        ...
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block4_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block4_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up4(h, emb)
        ...
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block3_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block3_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up3(h, emb)
        ...
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block2_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block2_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up2(h, emb)
        ...
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block1_1(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_block1_2(h, emb)

        new_skip = skips.pop()
        if new_skip is not None:
            current_skip = new_skip
        h = self._apply_skip_persistent(h, current_skip)
        h = self.dec_up1(h, emb)


        # ==================== OUTPUT ====================
        h = self.out_res1(h, emb)
        h = self.out_res2(h, emb)

        # Extract main feature from tuple
        if isinstance(h, tuple):
            h, _ = h

        dissection_logit = self.dissection_head(bottleneck_feat)
        
        return self.out(h),torch.sigmoid(dissection_logit)
        # return self.out(h)


    def _apply_skip_persistent(self, h, skip_feature):
        """
        Apply skip connections with persistent skip (like original code)

        Args:
            h: current feature (can be tuple of (feature, skip))
            skip_feature: current persistent skip connection

        Returns:
            tuple of (feature, skip) for frequency-aware processing
        """
        # Extract the main feature if h is a tuple
        if isinstance(h, tuple):
            h_main, _ = h
            # Replace the skip in tuple
            return (h_main, skip_feature)
        else:
            # h is not a tuple, create tuple with skip
            return (h, skip_feature)
